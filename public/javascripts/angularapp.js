  ...
  $routeProvider
    ...
    .when("/reservation", {
      ...
      resolve: [
        "auth",
        function (auth) {
          console.log("auth", auth, auth.islogin());
          auth.counttable();
          auth.islogin();
        },
      ],
...
app.factory("auth", [
  ...
  function ($http, $location, $rootScope) {
    ...
    auth.notable = function () {
      $http.get("/api/tables").success(function (response) {
        if (response === "12") $location.path("/reservation");
      });
    };
    auth.iscart = function () {
      $http
        .get("/cart/" + $rootScope.currentUser.username)
        .success(function (response) {
          $rootScope.existuser = response;
        });
    };
    auth.ischeck = function () {
      $http.get("/checkoutcart").success(function (response) {
        if (response === "0") $location.path("/menu");
      });
    };
    return auth;
  },
]);
app.controller("restcontrol", [
  "$scope",
  "$location",
  "$http",
  "$rootScope",
  "$window",
  function ($scope, $location, $http, $rootScope, $window) {
    $scope.user = {};
    $scope.payer = {};
    $scope.reserve = {};
    $scope.products = [];
    ...
    $scope.register = function () {
      if ($scope.user.rpassword === $scope.user.rrpassword) {
        ...
        } else $scope.er = "Password is weak";
      } else $scope.er = "Password is not same";
    };
    ...
    $scope.tablereserve = function () {
      $http
        .post("/api/reservetable", $scope.reserve)
        .error(function (error) {
    ...
    $scope.tablecancel = function () {
      $http
        .post("/api/canceltable/" + $scope.codename)
        .error(function (error) {
          console.log("Error: " + error);
        })
        .success(function (response) {
    ...
    $scope.checkout = function () {
      $http
        .post("/api/checkout/" + $rootScope.currentUser.username, $scope.payer)
        .error(function (error) {
          console.log("Error: " + error);
        })
        .success(function (response) {
    ...
    $scope.removeItem = function (itemid) {
      $http
        .post(
          "/api/removeItem/" + $rootScope.currentUser.username + "/" + itemid
        )
        .success(function (data) {
    ...
    $scope.reduceItem = function (itemid) {
      $http
        .post(
          "/api/reduceItem/" + $rootScope.currentUser.username + "/" + itemid
        )
        .success(function (data) {
          $rootScope.existuser = data;
          $http
    ...
    $scope.addtocart = function (item) {
      ...
      $http
        .post("/api/additem/" + $rootScope.currentUser.username, $scope.it)
        .success(function (data) {
          $rootScope.existuser = data;
        })
        .error(function (data) {
